...
// CipherPlan-HEIR fork: these two includes were removed upstream but are
// still needed by the HEIR_NO_YOSYS (#else) branch below, which calls
// oneShotBufferize / createRemoveUnusedMemRef. Upstream CI only builds
// with Yosys, so it never compiles that branch. We build
// --//:enable_yosys=0, so we re-add them. See heir-upstream-patches.md #3.
